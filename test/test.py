...
async def test_dice_activehighbuttons(dut):
  ...
  dut.cfg.value = 3'b001 # Configure buttons as active high, outputs as active low
  await reset(dut)
...
async def test_dice_activelowbuttons(dut):
  ...
  clock = Clock(dut.clk, 30, units="us") # Approximation of 32768 Hz
  cocotb.start_soon(clock.start())
  dut.cfg.value =  3'b000 # Configure buttons as active low, outputs as active low
  activeLevel=0
  commonLevel=0
  await reset(dut)
  digitsShown_task = cocotb.start_soon(checkDigitsShown(dut))
...
async def test_dice_activehighsegments(dut):
  ...
  clock = Clock(dut.clk, 30, units="us") # Approximation of 32768 Hz
  cocotb.start_soon(clock.start())
  dut.cfg.value = 3'b011 # Configure buttons as active high, segment outputs as active high
  await reset(dut)
  digitsShown_task = cocotb.start_soon(checkDigitsShown(dut))
  segmentsShown_task = cocotb.start_soon(checkSegmentOutputs(dut))
  dut._log.info("Running test")
...
async def test_dice_activehighcommons(dut):
  ...
  clock = Clock(dut.clk, 30, units="us") # Approximation of 32768 Hz
  cocotb.start_soon(clock.start())
  dut.cfg.value = 3'b101 # Configure buttons as active high, common outputs as active high
  await reset(dut)
  digitsShown_task = cocotb.start_soon(checkDigitsShown(dut))
  segmentsShown_task = cocotb.start_soon(checkSegmentOutputs(dut))
  dut._log.info("Running test")
...
async def test_dice_activehighboth(dut):
  ...
  clock = Clock(dut.clk, 30, units="us") # Approximation of 32768 Hz
  cocotb.start_soon(clock.start())
  dut.cfg.value = 3'b111 # Configure buttons as active high, common and segment outputs as active high
  await reset(dut)
  digitsShown_task = cocotb.start_soon(checkDigitsShown(dut))
  segmentsShown_task = cocotb.start_soon(checkSegmentOutputs(dut))
  dut._log.info("Running test")
